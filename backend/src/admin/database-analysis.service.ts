...
export class DatabaseAnalysisService {
  ...
  async analyzeSchema() {
    ...
    try {
      ...
      console.log(`   - Migration 表存在: ${schemaReport.migrationStatus.hasTable}`);
      ...
      return {
        ...
        summary: {
          ...
          executedMigrationsCount: schemaReport.migrationStatus.executedMigrations.length
        }
      };
...
`;
    ...
    if (this.schemaReport.enums.length > 0) {
      ...
      for (const enumType of this.schemaReport.enums) {
        const enumValues = enumType.enum_values.map((v: string) => `'${v}'`).join(', ');
  ...
  async resetMigrationHistory() {
    ...
    try {
      ...
      return {
        ...
        action: this.schemaReport.migrationStatus.hasTable ? 'cleared' : 'created'
      };

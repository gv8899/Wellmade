  ...
  async updateImageUrls() {
    try {
      ...
      for (const product of products) {
        ...
        if (
          product.imageUrl &&
          product.imageUrl.includes('wellmade.select/api/uploads/')
        ) {
        ...
        if (product.images && Array.isArray(product.images)) {
          ...
          if (
            JSON.stringify(updatedImages) !== JSON.stringify(product.images)
          ) {
        ...
        if (product.keyFeatures && Array.isArray(product.keyFeatures)) {
          const updatedKeyFeatures = product.keyFeatures.map((feature) => {
            if (
              feature.image &&
              feature.image.includes('wellmade.select/api/uploads/')
            ) {
              return {
                ...feature,
        ...
        if (product.featureDetails && Array.isArray(product.featureDetails)) {
          const updatedFeatureDetails = product.featureDetails.map((detail) => {
            if (
              detail.src &&
              detail.src.includes('wellmade.select/api/uploads/')
            ) {
              return {
                ...detail,
      ...
      for (const brand of brands) {
        if (
          brand.logoUrl &&
          brand.logoUrl.includes('wellmade.select/api/uploads/')
        ) {

...
export class AuthService {
  ...
  async validateUser(
    ...
    if (user) {
      this.logger.debug(
        `User found with email: ${email}. Validating password...`,
      );
  ...
  async login(
    ...
    if (!user) {
      this.logger.error(
        `Login failed for user with email: ${loginDto.email} - Unauthorized`,
      );
      throw new UnauthorizedException('電子郵件或密碼錯誤');
    }
    const payload = { email: user.email, sub: user.id, roles: user.roles };
    this.logger.debug(
      `Generating JWT for user with email: ${user.email} with payload: ${JSON.stringify(payload)}`,
    );
    const {
      id,
      username,
  ...
  async register(registerDto: RegisterDto): Promise<SafeUser> {
    this.logger.debug(
      `Registration attempt for user with email: ${registerDto.email}`,
    );
    try {
      ...
      this.logger.error(
        `Failed to register user with email ${registerDto.email}:`,
        error,
      );
  ...
  async validateOAuthLogin(profile: any): Promise<SafeUser> {
    this.logger.log(
      `收到 OAuth 登入請求，使用者: ${profile.email}, 資料:`,
